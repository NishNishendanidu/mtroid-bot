...
@borg.on(lightning_cmd(outgoing=True, pattern="username (.*)"))
...
@borg.on(lightning_cmd(outgoing=True, pattern="count$"))
...
@borg.on(lightning_cmd(outgoing=True, pattern=r"delpfp"))
...
@borg.on(lightning_cmd(pattern="myusernames$"))
async def _(event):
    if event.fwd_from:
        return
